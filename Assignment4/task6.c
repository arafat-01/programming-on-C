...
void printAllAverages (topThreeAvg *averages[], int studentSize, student *students, int entry_size, int all_ids[]) {
	printf ("FIRST\tSECOND\tID\tAVG\n");
	int i;
	int value[101];
	for (i = 0;i <= 100;i ++) value[i] = 0;
	for (i = 0;i < studentSize;i ++) 
		value[averages[i] -> ID] = averages[i] -> avgGrade;   // Save in the new array average grades for certain id

	for (i = 0;i <= 100;i ++) {
		if (all_ids[i] == 0) continue;        // We need to consider only valid ids
		int j;
		for (j = 0;j < entry_size;j ++) {
			if ((students + j) -> ID == i) {         // If it is id which we need, then print answer and break from the loop
				printf ("%s\t%s\t%i\t%i\n", (students + j) -> firstName, (students + j) -> secondName, i, value[i]);      
				break;
			}
		}        
	}
...
int main () {
	...
	int *cnt_id = (int*)malloc (101 * sizeof(int));   // Create all_ids[] array
	for (i = 0;i <= 100;i ++) cnt_id[i] = 0;          // Get rid of garbages

	student *pstudent = readStudents ("students1.txt", &lines, cnt_id, &howManyStudents); // Get pointer to the student array

	topThreeAvg *arr[howManyStudents];            // Create array of pointers for average with size 'studentSize'

	for (i = 0;i < howManyStudents;i ++) 
		arr[i] = (topThreeAvg*)malloc (sizeof(topThreeAvg));        // Initialize

	fillAllStudentsAvgGrades (arr, pstudent, lines, cnt_id);   // Fill array 'average'

	printAllAverages (arr, howManyStudents, pstudent, lines, cnt_id);  // Print answer
